...
class VibeVoiceStreamingForConditionalGenerationInference(VibeVoiceStreamingPreTrainedModel, GenerationMixin):
    ...
    def _build_generate_config_model_kwargs(self, generation_config, inputs, tokenizer, return_processors=False, **kwargs):
        ...
        self._prepare_cache_for_generation(generation_config, model_kwargs, None, batch_size, max_cache_length, self.device)
    ...
    def generate(
        ...
        **kwargs,
        ...
        while True:
            ...
            diffusion_indices = torch.nonzero(~finished_tags).squeeze(1)        
            # 만약 모든 배치가 끝났다면 루프 탈출
            ...
            for cur_speech_index in range(TTS_SPEECH_WINDOW_SIZE):
                ...
                # [핵심 수정] 현재 돌고 있는 모든 배치의 EOS 확률을 계산
                current_hidden_states = tts_lm_outputs.last_hidden_state[diffusion_indices, -1, :]
                tts_eos_logits = torch.sigmoid(self.tts_eos_classifier(current_hidden_states))
                
                # 확률이 0.5 넘는 녀석들 찾기 (배치 내 상대 인덱스)
                ended_indices_local = (tts_eos_logits.squeeze(1) > 0.5)
                
                # 실제로 끝난 글로벌 인덱스 찾기
                ended_indices_global = diffusion_indices[ended_indices_local]

                if ended_indices_global.numel() > 0:
                    # 끝난 녀석들만 True로 변경
                    finished_tags[ended_indices_global] = True
                    
                    # 스트리머가 있으면 끝난 녀석들만 종료 신호 보냄
                    if audio_streamer is not None:
                         audio_streamer.end(ended_indices_global)
